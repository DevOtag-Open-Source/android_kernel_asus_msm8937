...
int hx83102_i2c_error_count = 0;
int hx83102_irq_enable_count = 0;

extern struct hx_83102_ic_data* hx83102_ic_data;
extern struct hx_83102_ts_data *hx83102_ts;
extern void hx83102_ts_work(struct hx_83102_ts_data *ts);
extern enum hrtimer_restart hx83102_ts_timer_func(struct hrtimer *timer);

#ifdef HX_TP_PROC_DIAG
extern uint8_t hx83102_getDiagCommand(void);
#endif

extern void hx83102_log_touch_int_devation(int touched);


int hx83102_dev_set(struct hx_83102_ts_data *ts)
{
    int ret = 0;
...
int hx83102_input_register_device(struct input_dev *input_dev)
{
    return input_register_device(input_dev);
}

#if defined(HX_PLATFOME_DEFINE_KEY)
void hx83102_platform_key(void)
{
                     ...
                     struct himax_i2c_platform_data *pdata)
{
    ...
    struct hx_83102_virtual_key *vk;
...
int hx83102_parse_dt(struct hx_83102_ts_data *ts,
                   struct himax_i2c_platform_data *pdata)
{
...
int hx83102_i2c_himax_read(struct i2c_client *client, uint8_t command, uint8_t *data, uint8_t length, uint8_t toRetry)
{
    int retry;
    struct i2c_msg msg[] =
    {
    ...
    // printk("[Paul][3] i2c address = 0x%2x\n", client->addr);

    mutex_lock(&hx83102_ts->rw_lock);
    ...
    if (retry == toRetry)
    {
        E("%s: i2c_read_block retry over %d\n",
          __func__, toRetry);
        hx83102_i2c_error_count = toRetry;
		mutex_unlock(&hx83102_ts->rw_lock);
        return -EIO;
    }
    mutex_unlock(&hx83102_ts->rw_lock);
    return 0;
...
int hx83102_i2c_himax_write(struct i2c_client *client, uint8_t command, uint8_t *data, uint8_t length, uint8_t toRetry)
{
    int retry/*, loop_i*/;
...
int hx83102_i2c_himax_master_write(struct i2c_client *client, uint8_t *data, uint8_t length, uint8_t toRetry)
{
    int retry/*, loop_i*/;
    uint8_t buf[length];
...
void hx83102_int_enable(int irqnum, int enable)
{
    if (enable == 1 && hx83102_irq_enable_count == 0)
    {
        enable_irq(irqnum);
        hx83102_irq_enable_count++;
        hx83102_ts->irq_enabled = 1;
    }
    else if (enable == 0 && hx83102_irq_enable_count == 1)
    {
        disable_irq_nosync(irqnum);
        hx83102_irq_enable_count--;
        hx83102_ts->irq_enabled = 0;
    }
    I("hx83102_irq_enable_count = %d", hx83102_irq_enable_count);
}

#ifdef HX_RST_PIN_FUNC
void hx83102_rst_gpio_set(int pinnum, uint8_t value)
{
...
uint8_t hx83102_int_gpio_read(int pinnum)
{
...
int hx83102_gpio_power_config(struct i2c_client *client,struct himax_i2c_platform_data *pdata)
{
    int error;
...
static void himax_ts_isr_func(struct hx_83102_ts_data *ts)
{
    hx83102_ts_work(ts);
}
...
{
    struct hx_83102_ts_data *ts = ptr;

    if (ts->debug_log_level & BIT(2))
    {
        hx83102_log_touch_int_devation(HX_FINGER_ON);
    }
    ...
    if(ts->debug_log_level & BIT(2))
    {
        hx83102_log_touch_int_devation(HX_FINGER_LEAVE);
    }
    return IRQ_HANDLED;
}

static void hx83102_ts_work_func(struct work_struct *work)
{
    struct hx_83102_ts_data *ts = container_of(work, struct hx_83102_ts_data, work);
    hx83102_ts_work(ts);
}

static int himax_int_register_trigger(struct i2c_client *client)
{
    int ret = 0;
...
int hx83102_int_en_set(struct i2c_client *client)
{
    int ret = NO_ERR;
...
int hx83102_ts_register_interrupt(struct i2c_client *client)
{
    struct hx_83102_ts_data *ts = i2c_get_clientdata(client);
    int ret = 0;
    ...
    if (!ts->use_irq)  /*if use polling mode need to disable HX_ESD_RECOVERY function*/
    {
        ...
        INIT_WORK(&ts->work, hx83102_ts_work_func);

        hrtimer_init(&ts->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
        ts->timer.function = hx83102_ts_timer_func;
...
{
    struct hx_83102_ts_data *ts = dev_get_drvdata(dev);

    I("%s: enter \n", __func__);

    hx83102_chip_common_suspend(ts);
    return 0;
}
                         ...
                         unsigned long event, void *data)
{
    ...
    struct hx_83102_ts_data *ts=
        container_of(self, struct hx_83102_ts_data, fb_notif);
...
static struct i2c_driver himax_common_driver =
{
    ...
    .probe		= hx83102_chip_common_probe,
    .remove		= hx83102_chip_common_remove,
    .driver		= {
